...
public class ManyToOneAssociationTests {
    ...
    /* 설명.
     *  Association Mapping 은 Entity 클래스 간의 관계를 매핑하는 것을 의미한다. (join 을 위함)
     ...
     *  .
     *  2. 방향에 따른 분류
     *   1) 테이블의 연관 관계는 외래 키를 이용하여 양방향 연관 관계의 특징을 가진다.
     *      반면, 객체는 참조에 의한 연관관계로 단방향이다.
     *   2) 객체 간의 연관 관계를 양방향으로 만들고 싶을 경우 반대 쪽에서도 필드를 추가해서 참조를 보관하면 된다.
