...
public class ProblemOfUsingDirectSQLTests {
    ...
    // jbdcTemplate 의 getConnection 메소드 역할
    @BeforeEach
    ...
    /* 설명. JDBC APY를 이용해 직접 SQL 을 다룰 떄 발생할 수 있는 문제점
     *  1. 데이터 변환, SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하)
     *  2. SQL 에 의존하여 개발
     *  3. 패러다임 불일치(상속, 연관관계, 객체 그래프 탐색, 방향성)
     *  4. 동일성 보장 문제(독같은 메뉴를 조회해올 때마다 그걸 담은 객체는 매번 새로운 객체가 나옴.)
     * */
...
//            menu.setMenuPrice(rset.getInt("MENU_PRICE"));
    ...
    /* 설명. 3. 패러다임 불일치(상속, 연관관계, 객체 그래프 탐색, 방항성 -> 이 모두는 DB엔 없고 자바에만 있는 개념) */

    /* 설명. 3-1. 상속 문제
     *  객체 지향 언어의 상속 개념과 유사한 것이 데이터베이스의 서브타입 엔티티 이다.(서브타입을 별도의 클래스로 나뉘었을 때)
    ...
    /* 설명. 3-2. 연과관게 문제, 객체 그래프 탐색 문제
     *  객체지향에서 말하는, 가지고 있는(ASSOCIATION 연관관계 혹은 COLLECTION 연관관계) 경우 DB 저장 구조과 다른 형태이다.
     *
     * 설명.
    ...
    void testEquals() throws SQLException {
        ...
        // then
        Assertions.assertNotEquals(menu1, menu2);       // 주소값이 다르면 초록불

        /* 설명. JPA 를 이용하면 동일 비교가 가능하다.
