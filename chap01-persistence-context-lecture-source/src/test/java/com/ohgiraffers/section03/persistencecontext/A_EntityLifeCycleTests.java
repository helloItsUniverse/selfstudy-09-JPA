...
public class A_EntityLifeCycleTests {
    ...
    * */
    ...
    public void 비영속성_테스트() {
        Menu foundMenu = entityManager.find(Menu.class, 11);        // 영속 상태의 객체
//        System.out.println("foundMenu = " + foundMenu);

        /* 설명. 영속 상태의 값을 추출해 담더라도 새로 생성되어 영속성 컨텍스트와 관련없는 객체는 비영속 상태이다. */
...
//        entityManager.clear();
        ...
        /* 설명. 영속성 컨텍스트 및 엔티티 매니저까지 종료해버린다.(사용 볼가) */
        entityManager.close();

        foundMenu1.setMenuPrice(5000);
        foundMenu2.setMenuPrice(5000);

        /* 설명. DB에서 새로 조회해온 객체를 영속상태로 두기 때문에 전혀 다른 결과가 나온다. */
        assertEquals(5000, entityManager.find(Menu.class, 11).getMenuPrice());
        assertEquals(5000, entityManager.find(Menu.class, 12).getMenuPrice());
    }
    ...
    public void 병합_merge_수정_테스트() {
        ...
        entityManager.detach(menuToDetach);     // 잠깐 떼 놓은 다음에

        menuToDetach.setMenuName("수박죽");
        Menu reFoundMenu = entityManager.find(Menu.class, 2);       // refoundMenu 에는 기존의 메뉴 이름이 있다.
